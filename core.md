...
Каждый объект в Python начинается с заголовка `PyObject`, содержащего базовую информацию:

- `ob_refcnt` (счётчик ссылок) - отслеживает, сколько переменных ссылаются на объект. При достижении нуля память
  освобождается немедленно.
- `ob_type` (тип объекта) - определяет поведение объекта (какие методы доступны, как он обрабатывается). Содержит
  указатель на структуру `PyTypeObject`, которая описывает все операции для этого типа.

**Важные детали:**

- Заголовок занимает 16 байт в релизных сборках, 24 байта в отладочных (где `_PyObject_HEAD_EXTRA` добавляет
  дополнительные поля для отслеживания всех объектов)
- Система типов Python динамическая: `ob_type` можно изменить во время выполнения (что используется, например, в
  механизме `__class__`)
- Весь полиморфизм в Python реализован через проверку `ob_type` и вызов соответствующих функций из структуры типа
...
**PyTypeObject — метаструктура для типов в Python**

`PyTypeObject` — это фундаментальная структура, определяющая поведение каждого типа в Python. Каждый тип (класс) имеет свою собственную статическую структуру `PyTypeObject`, которая хранится в памяти как объект типа (например, `<class 'list'>`).

**Ключевые роли PyTypeObject:**
1. **Описание объектов типа** — содержит размер объекта (`tp_basicsize`, `tp_itemsize`), имя типа (`tp_name`), смещения к словарю и списку слабых ссылок
2. **Таблицы виртуальных функций** — указатели на специализированные функции для поддержки протоколов:
   - `PyNumberMethods` — математические операции (`+`, `-`, `*`, `/`)
   - `PySequenceMethods` — операции последовательностей (индексация, срезы)
   - `PyMappingMethods` — операции отображений (доступ по ключу)
3. **Управление жизненным циклом** — функции деструктора (`tp_dealloc`), инициализации, выделения памяти
4. **Работа с атрибутами** — функции получения/установки атрибутов, дескрипторы
5. **Наследование и разрешение методов** — MRO (`tp_mro`), базовые классы, кэширование атрибутов

**Особенности в Python 3.9+:**
- Типы реализованы как объекты (наследуются от `object`), поэтому `PyTypeObject` начинается с `PyVarObject`
- В `tp_dict` хранится пространство имен класса (методы, атрибуты)
- Многие слоты были добавлены для оптимизации (например, `tp_cache` для кэширования атрибутов в свободнопоточной реализации)
- Некоторые устаревшие поля (как `tp_print`) сохранены для обратной совместимости, но не используются
