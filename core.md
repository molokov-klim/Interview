...
`PyTypeObject` — это фундаментальная структура, определяющая поведение каждого типа в Python. Каждый тип (класс) имеет
свою собственную статическую структуру `PyTypeObject`, которая хранится в памяти как объект типа (например,
`<class 'list'>`).

**Ключевые роли PyTypeObject:**

1. **Описание объектов типа** — содержит размер объекта (`tp_basicsize`, `tp_itemsize`), имя типа (`tp_name`), смещения
   к словарю и списку слабых ссылок
...
- Многие слоты были добавлены для оптимизации (например, `tp_cache` для кэширования атрибутов в свободнопоточной
  реализации)
