...
На собесе любят спрашивать именно эту «петлю» `type`/`object` и связь Python‑уровневого `type` с `PyTypeObject` внутри
интерпретатора.

Если нужно, можно дальше разобрать конкретные примеры: как в C‑API создаётся свой тип, как работают `__slots__` (
оптимизация хранения атрибутов), чем `PyObject_HEAD` отличается от `PyObject_VAR_HEAD` и как разные категории типов
используют разные группы слотов.
...
Если спросят код — покажите `dis.dis('def f(a, *b, c=1, **d): pass')` и объясните offsets в `co_varnames`.
...
Хеш-таблица позволяет очень быстро находить, добавлять и удалять элементы. В Python они реализованы как **словари (dict)
** и **множества (set)**.
...
Если нужно, можно разобрать конкретные функции из `dictobject.c` (`lookdict`, `insertion`, split‑table для instance dict
и т.д.) и придумать 2–3 задачки «на доске» по моделированию коллизий.
...
**Итерируемый объект** — любой объект, который можно перебрать в цикле `for`. Он должен реализовывать метод `__iter__()`
или (для обратной совместимости) `__getitem__()`. Примеры: список, строка, словарь.

**Итератор** — объект для последовательного перебора элементов. Должен иметь метод `__next__()`, который возвращает
следующий элемент или вызывает `StopIteration` при завершении. Итератор также является итерируемым объектом (имеет
`__iter__()`, который возвращает сам себя).

**Генератор** — особый вид итератора, создаваемый функцией с ключевым словом `yield` или генераторным выражением.
Главное преимущество — ленивое вычисление (lazy evaluation): значения генерируются по запросу, а не хранятся в памяти
целиком.
...
### 1. **Протокол итератора: детали реализации**

Класс, реализующий протокол итератора:
...
### 2. **Генераторные функции: механизм работы**

При вызове генераторной функции возвращается объект-генератор, но код функции не выполняется сразу. Выполнение
начинается при первом вызове `next()` и приостанавливается на `yield`. Состояние (локальные переменные, точка
выполнения) сохраняется между вызовами.

```python
def generator_with_return():
    yield 1
    yield 2
    return "Done"  # Значение присоединяется к StopIteration
...
### 3. **Методы генераторов: двусторонняя коммуникация**

Генераторы поддерживают взаимодействие через:

- `.send(value)` — отправляет значение, которое становится результатом `yield`
- `.throw(exc_type[, exc_value[, traceback]])` — вызывает исключение в точке `yield`
- `.close()` — вызывает `GeneratorExit` внутри генератора
...
gen.close()  # Останавливаем генератор
```

### 4. **`yield from`: делегирование генераторов**

Выражение `yield from` (Python 3.3+) упрощает делегирование генерации значений:
...
list(chain_generators())  # [0, 1, 2, 0, 2, 4]
```

`yield from` также передает управление вложенному генератору, включая вызовы `.send()`, `.throw()` и `.close()`.

### 5. **Генераторные выражения и оптимизация памяти**

Генераторные выражения похожи на списковые включения, но используют круглые скобки и не создают список целиком:
...
### 6. **Особенности итераторов и генераторов**

- **Одноразовость**: большинство итераторов (включая генераторы) можно использовать только один раз.
- **Исключения в генераторах**: исключение, возникшее в генераторе, приводит к его завершению и не может быть обработано
  внутри после этого.
- **Производительность**: генераторы экономят память, но могут быть медленнее циклов из-за накладных расходов на вызовы.
- **Отличие от итераторов**: любой генератор — итератор, но не наоборот. Генераторы создаются функциями с `yield`,
  итераторы — классами с `__next__()`.

### 7. **Практические применения**

- **Чтение больших файлов**: построчная обработка без загрузки всего файла.
- **Конвейеры обработки данных**: цепочки генераторов для потоковой обработки.
- **Генерация бесконечных последовательностей**: числа Фибоначчи, простые числа.
- **Асинхронное программирование**: основа `asyncio` до появления `async/await`.

Генераторы и итераторы — фундаментальные концепции Python, обеспечивающие эффективную работу с последовательностями и
реализацию шаблонов отложенных вычислений.

## **Senior Level (CPython, байткод и системные структуры)**

Генераторы и итераторы в CPython завязаны на единый протокол итерации и довольно тонко интегрированы в интерпретатор:
через слоты типов, фреймы, байткод и специальные объектные структуры. Для собеседования на Core Python важно уметь
рассказать и “на уровне протокола”, и “на уровне С‑структур и байткода”.

## Базовые определения

Итерируемый объект — любой объект, от которого встроенная функция `iter()` может получить итератор. Итератор — объект,
реализующий протокол: методы `__iter__()` (возвращает самого себя) и `__next__()` (возвращает следующий элемент или
бросает `StopIteration`). Генератор — частный случай итератора, создаваемый либо функцией с `yield`, либо генераторным
выражением; объект‑генератор реализует `__iter__` и `__next__`, поэтому любой генератор в Python является
итератором.

## Протокол итерации на уровне Python

Функция `iter(obj)` действует по следующему алгоритму:

- Если у объекта есть метод `__iter__`, вызывается он и ожидается объект‑итератор.
- Если `__iter__` отсутствует, но есть `__getitem__` с поддержкой индексации с нуля, создаётся итератор, который
  вызывает `obj[i]`, увеличивая индекс, пока не будет выброшено `IndexError`.
- Если не реализован ни один из этих методов, выбрасывается `TypeError`.

Функция `next(it, default)` вызывает метод `it.__next__()`; при `StopIteration` либо пробрасывает исключение, либо
возвращает `default`, если он передан. Цикл `for` реализуется как:

- Получить итератор `it = iter(iterable)`.
- Повторять `next(it)` до `StopIteration`.

## Реализация итераторов в CPython

На уровне C каждый тип (`PyTypeObject`) имеет два поля, отвечающих за протокол итерации:

- `tp_iter` — функция, возвращающая итератор (аналог `__iter__`).
- `tp_iternext` — функция, возвращающая следующий элемент (аналог `__next__`).

Для “классических” коллекций:

- Списки, кортежи, словари и их ключи/значения имеют отдельные C‑типы итераторов (`listiter`, `tupleiter`, `dictkeyiter`
  и т.п.), хранящие ссылку на исходный контейнер и текущий индекс/позицию.
- При изменении контейнера некоторые итераторы либо корректно продолжают обход (например, список при добавлении), либо
  защищаются от некорректного состояния, используя версию/счётчик модификаций.

С точки зрения Python‑разработчика, кастомный итератор повторяет ту же модель: объект хранит состояние (позицию),
реализует `__iter__` и `__next__`, где последний либо возвращает следующий элемент, либо бросает `StopIteration`.

## Генераторы: объект и фрейм

Функция с `yield` при компиляции получает другой тип кода: вместо обычной функции создаётся “генераторная функция”,
которая при вызове не исполняет тело сразу, а возвращает объект‑генератор. Объект‑генератор внутри CPython
содержит:

- Указатель на фрейм выполнения (в новых версиях — встроенный `_PyInterpreterFrame`), в котором лежат локальные
  переменные, стек, счётчик текущей инструкции и состояние исключений.
- Служебные поля: статус (создан, выполняется, завершён), ссылку на код‑объект, ссылку на “создавший” интерпретатор и
  т.п..

Важно: в отличие от обычной функции, фрейм генератора **не уничтожается** после возврата значения, а “замораживается” и
хранится в объекте генератора до следующего вызова `next()` или `send()`. Это и даёт “паузу” между `yield` и
продолжением выполнения.

## Байткод и `yield`

При компиляции генераторной функции интерпретатор генерирует специальную последовательность байткодов:

- В ранних версиях — опкоды вроде `YIELD_VALUE` и `GET_YIELD_FROM_ITER`; в новых — отдельная документация описывает
  логику вокруг `YIELD_VALUE` и `SEND`.
- Инструкция `YIELD_VALUE` берёт верхнее значение со стека фрейма и возвращает его вызывающему коду, одновременно
  помечая фрейм как приостановленный.
- При последующем `next(gen)` или `gen.send(value)` интерпретатор возобновляет выполнение фрейма с той инструкции,
  которая следует за `YIELD_VALUE`.

Ключевые шаги при вызове `next(gen)`:

- Проверка, не завершён ли генератор.
- Восстановление фрейма (локальные переменные, стек, состояние исключений).
- Продолжение интерпретации байткода до следующего `YIELD_VALUE` или завершения функции (`RETURN_VALUE`), где при
  завершении генерируется `StopIteration` с возможным значением `value` (результат `return` в генераторе).

## `yield from` и протокол передачи значений

Конструкция `yield from subgen` раскладывается компилятором в управляемый цикл, который делегирует итерацию вложенному
генератору/итератору. На уровне байткода:

- Для делегирования используется комбинация инструкций, среди которых ключевую роль играет `SEND`: она отправляет
  значение во вложенный генератор и получает либо очередное значение, либо сигнал о завершении (`StopIteration`).
- Цикл продолжается, пока вложенный генератор не завершится; его итоговое значение (`StopIteration.value`) передаётся
  наружу как результат `yield from`.

Эта схема полностью описана в внутренней документации CPython по генераторам и позволяет реализовать корутины и
асинхронные конструкции поверх генераторов (исторически `async`/`await` базировались именно на протоколе
генераторов).

## Взаимодействие с протоколом итерации

Объект‑генератор полностью соответствует протоколу итератора:

- `__iter__()` возвращает самого себя.
- `__next__()` вызывает механизм возобновления фрейма и возвращает очередное значение или бросает `StopIteration`.

Поэтому:

- Цикл `for` не отличает “обычный” итератор от генератора.
- Генератор можно использовать везде, где ожидается итератор, в том числе в `itertools`, `sum`, `list`, генераторных
  выражениях и comprehension.

## Итераторы, генераторы и память/производительность

Главное практическое различие: итераторы и особенно генераторы позволяют “лениво” получать элементы, не держа всю
последовательность в памяти. С точки зрения CPython:

- Хранится только состояние фрейма генератора (локальные переменные и стек), а не вся коллекция.
- У встроенных итераторов коллекций состояние ещё компактнее: индекс/позиция плюс ссылка на контейнер.

Это даёт:

- Возможность работать с очень длинными или бесконечными последовательностями (например, `itertools.count`).
- Снижение давления на GC: меньше крупных временных объектов (списки и т.д.), больше компактных
  итераторов/генераторов.

Если нужно, можно на собеседовании дополнительно упомянуть:

- Что все эти объекты — полноценные `PyObject` с подсчётом ссылок и участием в циклическом GC.
- Что для разных встроенных типов используются разные специализированные C‑итераторы (по ключам словаря, по значениям,
  по элементам множества и т.п.).
