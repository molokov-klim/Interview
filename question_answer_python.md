...
**Декоратор** — это функция, которая принимает другую функцию и возвращает новую, изменяя или дополняя её поведение без
модификации исходного кода. Синтаксически декоратор применяется с помощью символа `@`.

**Замыкание** — функция, которая запоминает переменные из внешней области видимости, даже когда эта внешняя функция
завершила выполнение. Замыкания позволяют создавать функции с "памятью".
...
### 1. **Механизм декораторов**

При использовании синтаксиса `@decorator` Python выполняет:
...
- Функцией, возвращающей функцию
- Классом с методом `__call__`
- Цепочкой декораторов (применяются снизу вверх)
...
### 3. **Сохранение метаданных с `functools.wraps`**

Без `wraps` декорированная функция теряет имя, документацию и другие атрибуты:
...
def decorator(func):
    @wraps(func)  # Критически важная строка
    def wrapper(*args, **kwargs):
        return func(*args, **kwargs)
...
Класс становится декоратором при реализации метода `__call__`:
...
example()
print(example.calls)  # 1
```

### 5. **Замыкания и область видимости**

Python использует правило **LEGB** для поиска переменных:

- **L**ocal — локальная область
- **E**nclosing — включающая (замыкания)
- **G**lobal — глобальная
- **B**uilt-in — встроенная

Ключевое слово `nonlocal` позволяет изменять переменные из замыкания:
...
def make_accumulator():
    ...
    def add(x):
        nonlocal total  # Без этого будет UnboundLocalError
        total += x
        return total
...
### 6. **Декораторы методов класса**

При декорировании методов нужно учитывать, что первый аргумент (`self`) должен корректно передаваться:
...
def flexible_decorator(func=None, *, option=True):
    if func is None:
        return lambda f: flexible_decorator(f, option=option)

    @wraps(func)
    def wrapper(*args, **kwargs):
        if option:
            print("Опция включена")
        return func(*args, **kwargs)
...
@flexible_decorator(option=False)
def f2(): pass
```

**Несколько декораторов (цепочка):**
...
@decorator3
def func(): ...
# Эквивалентно: decorator1(decorator2(decorator3(func)))
```

**Декораторы с доступом к аргументам:**
...
### 8. **Применение в тестировании и фреймворках**

- **`@pytest.fixture`** — создание тестовых данных
- **`@pytest.mark.parametrize`** — параметризация тестов
- **`@unittest.mock.patch`** — временная замена объектов
- **`@staticmethod`**, **`@classmethod`** — встроенные декораторы классов
- **`@property`**, **`@x.setter`**, **`@x.deleter`** — управление доступом к атрибутам
- **`@lru_cache`** (из `functools`) — мемоизация результатов функции

### 9. **Производительность и ограничения**

- Декораторы добавляют накладные расходы на вызов функции
- Замыкания потребляют память для хранения захваченных переменных
- Стек вызовов становится глубже, что может усложнить отладку
- Для интроспекции (получения сигнатуры функции) всегда используйте `@wraps`

Декораторы и замыкания — мощные инструменты метапрограммирования в Python, которые позволяют создавать чистый,
переиспользуемый и модульный код, следуя принципам DRY (Don't Repeat Yourself) и обеспечивая разделение ответственности.
