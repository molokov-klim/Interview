...
Типы данных в Python делятся на **изменяемые** (mutable) и **неизменяемые** (immutable). Ключевое отличие в том, что
изменяемые объекты (`list`, `dict`, `set`, `bytearray`) можно модифицировать после создания, а любые операции над
неизменяемыми (`int`, `float`, `decimal`, `complex`, `str`, `bytes`, `tuple`, `frozenset`, `bool`, `NoneType`) создают
новый объект. Это фундаментальное различие влияет на поведение при передаче в функции (для mutable изменения видны
снаружи), возможность быть ключом словаря (требуется хэшируемость) и работу механизмов памяти.

## **Junior Level**

**Простые (скалярные) типы:** числа (`int`, `float`, `complex`), строки (`str`) и логические значения (`bool`). Их
ключевое свойство — неизменяемость. Операция `x = "hello"; x = "world"` не меняет исходную строку, а создаёт новую и
переназначает имя `x`.

**Коллекционные типы:** предназначены для хранения наборов значений.

* `list` — изменяемая упорядоченная коллекция.
* `tuple` — неизменяемая упорядоченная коллекция.
* `dict` — изменяемая коллекция пар «ключ-значение», неупорядоченная (порядок гарантирован с Python 3.7).
* `set` / `frozenset` — изменяемое и неизменяемое множества уникальных элементов.

## **Middle Level**

1. **Классификация по мутабельности:**
    * **Изменяемые:** `list`, `dict`, `set`, `bytearray`, пользовательские классы. При передаче в функцию передаётся
      ссылка на объект.
    * **Неизменяемые:** `int`, `float`, `decimal`, `complex`, `str`, `bytes`, `tuple`, `frozenset`, `bool`, `NoneType`.
      Объект нельзя изменить после создания.

2. **Устройство объектов:** Каждый объект имеет идентификатор (`id()`), тип (`type()`) и значение. Неизменяемый объект
   жестко связывает идентификатор и значение. Изменяемый объект может менять значение, сохраняя идентификатор.

3. **Специфика типов:**
    * `None` — синглтон, обозначающий отсутствие значения.
    * Булевы значения `True` и `False` — синглтоны и подклассы `int`.
    * `tuple` — неизменяем, но если содержит изменяемые элементы, они могут быть изменены. Это влияет на хэшируемость
      кортежа.
    * `set` и `frozenset` хранят только хэшируемые элементы. Их реализация близка к словарю без значений.

## **Senior Level**

1. **Всё есть `PyObject`:** В CPython (`Include/object.h`) любая сущность — это структура `PyObject`, содержащая счётчик
   ссылок (`ob_refcnt`) для сборки мусора и указатель на тип (`ob_type`). Конкретные типы расширяют эту структуру (
   например, `PyLongObject` для целых чисел).

2. **Неизменяемость и интернирование (interning):** Оптимизация на уровне интерпретатора.
    * **Маленькие целые числа** (диапазон -5..256) — синглтоны, создаваемые при старте.
    * **Строки** (особенно имена) могут быть автоматически или принудительно (`sys.intern()`) интернированы для
      ускорения сравнения и экономии памяти.

3. **Мутабельность и байткод:** Операция `my_list.append(x)` компилируется в инструкцию `LIST_APPEND`, которая напрямую
   работает с внутренним C-массивом структуры `PyListObject`, меняя его размер и содержимое. Новый объект списка не
   создаётся.

4. **Словарь: краеугольный камень языка.** Реализован как хэш-таблица с методом открытой адресации. Используется в
   основе пространств имён, атрибутов объектов, `**kwargs`. Для AQA важно помнить о потенциальных проблемах: замедление
   при коллизиях (DoS-риск) и фрагментации памяти в долгоживущих объектах.

5. **`tuple` vs `list`: разница в памяти.** `tuple` аллоцируется как непрерывный блок памяти фиксированного размера.
   `list` содержит буфер с запасом (`allocated`). Пустые или одноэлементные кортежи могут кэшироваться и
   переиспользоваться.

6. **Типизация для AQA:** Python — язык с сильной динамической типизацией. «Утиная типизация» реализуется через поиск
   атрибута в `__dict__` и по цепочке MRO. На уровне C этому соответствуют вызовы `PyObject_IsInstance` и
   `PyObject_HasAttr`. Понимание этого критично для тестирования полиморфных компонентов и создания сложных
   мок-объектов.
